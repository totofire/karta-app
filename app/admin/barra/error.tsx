...
export default function BarraError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[KDS Barra] Error:", error);
  }, [error]);

  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center gap-6 text-white p-8">
      <div className="bg-blue-600/20 border border-blue-500/30 rounded-2xl p-8 max-w-sm w-full text-center">
        <GlassWater size={48} className="mx-auto mb-4 text-blue-400 opacity-60" />
        <h2 className="text-xl font-black mb-2">KDS Barra caído</h2>
