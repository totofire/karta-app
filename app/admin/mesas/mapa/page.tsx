...
// ─── ZONA MANUAL (drag + resize independiente de las mesas) ──────────────────
function ZonaEditor({
  nombre, rect, color, seleccionada, bloqueada, onSelect, onChange,
}: {
  ...
  // Resize desde cualquier handle cardinal + diagonal
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          position: "absolute", width: rect.w, height: rect.h,
      ...
      >
        ...
        <div style={{
          ...
          {bloqueada && (
            <span style={{
              background:"#374151", color:"#fff", borderRadius:20,
...
function MesaDraggable({ mesa, onDragStop, colorBorde }: {
  ...
  return (
    <Draggable
      ...
      bounds="parent" grid={[20, 20]}
    >
      <div ref={nodeRef} className="absolute cursor-grab active:cursor-grabbing group"
        style={{ width: 96, height: 96, zIndex: 50 }}>
        <div className="w-full h-full rounded-2xl flex flex-col items-center justify-center relative select-none"
          ...
          <Move size={10} className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-40 transition-opacity"
            style={{ color: colorBorde }} />
          <span className="font-black text-gray-800 text-sm leading-none">{mesa.nombre}</span>
          {mesa.sector && (
            <span className="text-[9px] font-bold uppercase tracking-wider mt-1 opacity-50"
              style={{ color: colorBorde }}>{mesa.sector}</span>
          )}
        </div>
      </div>
    </Draggable>
...
export default function EditorMapaPage() {
  ...
  // Mapa nombre → color (índice estable por orden en BD)
  ...
  useEffect(() => {
    Promise.all([
      ...
      // Generar posiciones por defecto para zonas que no tienen layout guardado
  ...
  return (
    <div className="h-full w-full bg-gray-200 flex flex-col overflow-hidden" style={{ minHeight: 0 }}>

      {/* HEADER */}
      <div className="bg-white px-5 py-3 border-b shadow-sm z-50 shrink-0 flex items-center gap-4">
        <Link href="/admin/mesas" className="p-2 hover:bg-gray-100 rounded-full text-gray-500 transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div>
            ...
            return (
              ...
              >
                ...
                <button onClick={e => { e.stopPropagation(); toggle(bloqueadas, setBloqueadas, s.nombre); }}
                  className="opacity-70 hover:opacity-100 transition-opacity" title={bloq ? "Desbloquear" : "Bloquear posición"}>
      ...
      {/* BARRA DE AYUDA */}
      <div className="bg-white border-b px-5 py-2 flex items-center gap-8 text-[11px] text-gray-400 font-medium shrink-0">
        <span className="flex items-center gap-1.5"><Move size={12}/> Arrastrá la <b>zona</b> para moverla</span>
        <span>Los <b>handles blancos</b> en los bordes cambian el tamaño de la zona</span>
        <span><Lock size={11} className="inline mr-1"/>Bloqueá la zona para mover mesas sin moverla</span>
      </div>

      {/* CANVAS */}
      <div className="flex-1 overflow-hidden relative">
      <div className="absolute inset-0 overflow-auto" onClick={() => setSeleccionada(null)}>
        {/* Cuadrícula */}
        ...
        <div className="relative w-[3000px] h-[3000px]">
          ...
          {/* ZONAS (capa inferior, completamente independiente de mesas) */}
          {sectores.map(s => {
          ...
          {/* MESAS (capa superior, totalmente independientes) */}
          {mesas.map(mesa => (
            <MesaDraggable
              key={mesa.id}
              mesa={mesa}
